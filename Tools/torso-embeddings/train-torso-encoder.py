...
def main() -> int:
    ...
    if args.resume and args.checkpoint_dir:
        ckpts = sorted(args.checkpoint_dir.glob("torso_checkpoint_epoch*.pt"))
        if ckpts:
            latest = ckpts[-1]
    ...
    print(f"Training on {len(dataset)} torso crops, epochs {start_epoch+1}..{args.epochs}, batch={args.batch_size}", flush=True)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.checkpoint_dir and ((epoch + 1) % 5 == 0 or epoch == args.epochs - 1):
            ckpt_path = args.checkpoint_dir / f"torso_checkpoint_epoch{epoch:03d}.pt"
            torch.save({
                "epoch": epoch,
